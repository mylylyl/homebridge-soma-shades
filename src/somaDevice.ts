...
export class SOMADevice {
    ...
    constructor(log: Logger, peripheral: noble.Peripheral) {
        ...
        this.initialized = false; // are characteristics initialized
    }

    public connect(): Promise<void> {
        return new Promise((resolve, reject) => {
            // Check the connection state
            let state = this.peripheral.state;
            if (state === 'connected') {
                this.connected = true;
                resolve();
                return;
    ...
    async getInfomationCharacteristics(): Promise<SOMADeviceInformation> {
        ...
        let deviceInfo: SOMADeviceInformation = {
        ...
        let services = await this.peripheral.discoverServicesAsync([INFO_SERVICE_UUID]);
        if (!services || services.length != 1 || services[0].uuid !== INFO_SERVICE_UUID) {
            this.log.error('Invalid service discovered');
            return deviceInfo;
        }

        let characteristics = await services[0].discoverCharacteristicsAsync();
    ...
    private async getCharacteristics(): Promise<void> {
        ...
        let services = await this.peripheral.discoverServicesAsync([MOTOR_SERVICE_UUID]);
        ...
        let motorCharacteristics = await services[0].discoverCharacteristicsAsync([MOTOR_STATE_CHARACTERISTIC_UUID, MOTOR_TARGET_CHARACTERISTIC_UUID, MOTOR_CONTROL_CHARACTERISTIC_UUID]);
    ...
    async setTargetPosition(position: number): Promise<void> {
        ...
        .catch((error) => {
            this.log.error('[setTargetPosition] error: %s', error)
        })
    }
    ...
    async setMotorStop(): Promise<void> {
        ...
        Promise.race([
            ...
            this.log.error('[setMotorStop] error: %s', error)
        })
